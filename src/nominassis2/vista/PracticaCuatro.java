...
public class PracticaCuatro {
    ...
    public void practicaCuatro(LinkedList<Nomina> listaNominas, String fecha, File excel) throws FileNotFoundException, MalformedURLException, ObjectNotFoundException, IOException {
        ...
        for (Nomina nomina : listaNominas) {
            if (!nomina.getTrabajadorbbdd().getNifnie().equals("")) {
                esExtra = false;
    ...
    private void ejercicio2(LinkedList<Nomina> listaNominas, File excel) throws IOException {
        ...
        idEmpresa = Coordinador.getCoordinador().getContadorIDEmpresas();
        for (Nomina nomina : listaNominas) {
            // PARTE EMPRESAS
        ...
        LinkedList<Trabajadorbbdd> trabajadoresNomina = new LinkedList<>();
        ...
        for (Nomina nomina : listaNominas) {
            if (!nomina.getTrabajadorbbdd().getNifnie().equals("")) {
                ...
                trabajadoresNomina.add(trabajador);
            }
        }

        int idNomina = Coordinador.getCoordinador().getIdNomina();
        for (Nomina nomina : listaNominas) {
            if (!nomina.getTrabajadorbbdd().getNifnie().equals("")) {
                // PARTE NOMINAS
                nomina.setTrabajadorbbdd(getTrabajador(trabajadoresNomina, nomina.getTrabajadorbbdd().getNifnie()));
